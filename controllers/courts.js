...
module.exports.createCourt = async (req, res, next) => {
    ...
    newCourt.images = req.files.map(img => ({ url: img.path, filename: img.filename }));
    await newCourt.save();
...
module.exports.showCourt= async (req, res) => {
    const foundCourt = await Court.findById(req.params.id).populate({
        path: 'reviews',
        populate: {
            path: 'author'
        }
...
module.exports.updateCourt = async (req, res) => {
    const updatedCourt = await Court.findByIdAndUpdate(req.params.id, { ...req.body.court });
    ...
    if (req.body.deleteImages) {
        await updatedCourt.updateOne({ $pull: { images: { filename: { $in: req.body.deleteImages } } } });
    }
