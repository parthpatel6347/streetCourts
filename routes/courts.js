...
const { storage, cloudinary } = require("../user-images");
const upload = multer({ storage });

router.route('/')
    .get(catchAsync(courts.index))
    .post(isLoggedIn, upload.array('image'), joiValidation, catchAsync(courts.createCourt));

router.get("/new", isLoggedIn, courts.renderNewForm);

router.route('/:id')
    .get(catchAsync(courts.showCourt))
    .put(isLoggedIn, isAuthor, upload.array('image'), joiValidation, catchAsync(courts.updateCourt))
